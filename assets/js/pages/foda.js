...
firebase.auth().onAuthStateChanged(res => {
    ...
    .get().then(el=>{
        var inf=el.data();
        basededatos=inf.bd;
        firebase.firestore().collection(basededatos+"/Direccion/Fodas").onSnapshot(el1=>{
            // var inf=el.data();
            var text='';
            el1.forEach(key => {
                var fila=key.data();
...
function gurdar(){
    ...
    if(nombre.value!='' && area.value!=''){
        ...
        firebase.firestore().collection(basededatos+"/Direccion/Fodas").add({
            Nombre : nombre.value,
            Area : area.value,
