...
firebase.auth().onAuthStateChanged(res => {
    ...
    .get().then(el=>{
        ...
        firebase.firestore().collection(basededatos+"/Direccion/MatrizComunicacionInterna").onSnapshot(el1=>{
            var text='';
            contador=0;
            el1.forEach(key => {
                var fila=key.data();
...
function abrirModalM(id){
    ...
    if(id=='+'){
        ...
        firebase.firestore().collection(basededatos+"/Direccion/MatrizComunicacionInterna").doc(id).get().then(res =>{
            var fila=res.data();
...
function gurdarmtz(){
    ...
    quien.value!='' && evidencia.value!=''){
        ...
        if(id_action=='+'){
            firebase.firestore().collection(basededatos+"/Direccion/MatrizComunicacionInterna").add({
            ...
            firebase.firestore().collection(basededatos+"/Direccion/MatrizComunicacionInterna").doc(id_action).update({
                Quecomunicar: quecomunicar.value,
                Cuando: cuando.value,
                Aquien: aquien.value,
                Como: como.value,
...
function eliminarmtz(){
    ...
    if(confirmacion){
        firebase.firestore().collection(basededatos+"/Direccion/MatrizComunicacionInterna").doc(id_action).delete()
        .then(function() {
            cerrarmodalmtz.click();
